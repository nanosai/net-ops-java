...
public class IapMessageReader implements IMessageReader {
    ...
    private IBytesAllocator readMemoryAllocator = null;
    ...
    private IapMessage currentMemoryBlock = null;
    ...
    public void init(IBytesAllocator readMemoryAllocator, ObjectPool<IapMessage> readObjectPool) {
        ...
        this.readObjectPool      = readObjectPool;
    }
        ...
        while(byteBuffer.hasRemaining()){
            switch(this.readStatus){
                case STATUS_NOTHING_READ:  {
                    this.currentMemoryBlock = this.readObjectPool.instance(); //allocate new Message object

                    int leadByte = 255 & byteBuffer.get();
                    fieldType    = leadByte >> 4;

                    if(fieldType != RionFieldTypes.OBJECT){
                ...
                case STATUS_LEAD_BYTE_READ: {
                    ...
                    if(this.lengthBytesRead == this.lengthLength){
                        ...
                        this.currentMemoryBlock.resetReadAndWriteIndexes();
